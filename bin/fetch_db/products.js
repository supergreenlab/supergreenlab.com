...
module.exports.fetchProducts = async () => {
  ...
  let collectionProducts = await fetchTable('CollectionProducts', ['slug', 'Product', 'order', 'Collections'])
  let collections = await fetchTable('Collections', ['slug', 'name','picture'])
  ...
  // collectionProducts = collectionProducts.map(cp => {
  //   cp.Collections = collections.filter(u => cp.Collections.indexOf(u) !== -1)
  // })
